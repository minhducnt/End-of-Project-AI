...
//   ĐÂY LÀ THUẬT TOÁN A*  MÀ TÔI NGÀY ĐÊM NGHIÊN CỨU    == > MINH BK  ....(GHI NHỚ CÔNG ƠN TÁC GIẢ : MINH BK)...    
    
    //CHÚ Ý: Trong game thì posW tính theo chiêu ngang, posH tính theo chiều dọc
    // Còn với mảng, nodeList[i][j] , b[i][j]  thì i tính theo chiều dọc, j tính theo chiều ngang...khó nhai r
	...
	ArrayList<Node> open = new ArrayList<Node>();
	    ...
	    closed.add(currentNode); 
            if (currentNode == target)
             ...
             for(int i = currentNode.getFX() - 1; i <= currentNode.getFX() + 1; i++) {      //nghĩa là các giá trị cận kề,theo chiều dọc
                if(i < 0 || i > DoRongDai.countH-1) {            //nếu <0 và > 20 thì bỏ qua
		    continue;
	        }
                for(int j = currentNode.getFY() - 1; j <= currentNode.getFY() + 1; j++) { //nghĩa là các giá trị cận kề, theo chiều ngang
